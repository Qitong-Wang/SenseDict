...
def replace_sense(input_emb, sense_emb, type, pca=None, pca_emb=None):
    ...
    if type == "dot":
        ...
        diff = sense_emb - input_emb  # Broadcasting subtracts A from each row of B
        ...
        target_reduced = pca.transform(input_emb.reshape(1, -1).detach().cpu().numpy())
        target_reduced_tensor = torch.from_numpy(target_reduced)
        result = torch.matmul(pca_emb, target_reduced_tensor.T).squeeze()
        index = torch.argmax(result)
